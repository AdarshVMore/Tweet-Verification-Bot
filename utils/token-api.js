...
const SubstreamsUtil = require('./substreams');
...
class TokenAPIUtil {
  constructor() {
    ...
    this.useSubstreamsFirst = process.env.USE_SUBSTREAMS_FIRST !== 'false';
  }
  ...
  async getProtocolTVL(protocol, chain = 'ethereum') {
    if (this.useSubstreamsFirst) {
      try {
        const substreamsResult = await this.substreamsUtil.getProtocolTVL(protocol, chain);
        if (substreamsResult) {
          return substreamsResult;
        }
      } catch (substreamsError) {
        console.warn(`Substreams TVL query failed for ${protocol}:`, substreamsError.message);
        console.warn('Falling back to Token API...');
      }
    }
    ...
    try {
      ...
      throw new Error(`Failed to get protocol TVL from both Substreams and Token API: ${error.message}`);
    }
  }
  ...
  async getTokenBalances(walletAddress, chain = 'ethereum') {
    if (this.useSubstreamsFirst) {
      try {
        const substreamsResult = await this.substreamsUtil.getTokenBalances(walletAddress, chain);
        if (substreamsResult) {
          return substreamsResult;
        }
      } catch (substreamsError) {
        console.warn(`Substreams balance query failed:`, substreamsError.message);
        console.warn('Falling back to Token API...');
      }
    }
    ...
    try {
      ...
      throw new Error(`Failed to get token balances from both Substreams and Token API: ${error.message}`);
    }
  }
  ...
  async getTokenTransfers(tokenAddress, walletAddress = null, chain = 'ethereum', limit = 10) {
    if (this.useSubstreamsFirst) {
      try {
        const substreamsResult = await this.substreamsUtil.getTokenTransfers(tokenAddress, walletAddress, chain, { limit });
        if (substreamsResult) {
          return substreamsResult;
        }
      } catch (substreamsError) {
        console.warn(`Substreams transfers query failed:`, substreamsError.message);
        console.warn('Falling back to Token API...');
      }
    }
    ...
    try {
      ...
      throw new Error(`Failed to get token transfers from both Substreams and Token API: ${error.message}`);
    }
  }
